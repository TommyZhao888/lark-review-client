...
| 一键自更新 | git pull + 重启 | **菜单栏「更新并重启」**（从 GitHub Releases 下载新版 dmg **原地替换自己** + 重启，任意安装位置可用）；「空闲时自动更新」**默认开启** |
...
跑完再更；更新中途来了 review 会中止更新让 review 先跑）。设置 → 更新里的 **「空闲时自动更新」**
**自 v1.10.4 起默认开启**（老配置首次跑新版会被一次性打开；关掉后不再翻回）：连上服务端且空闲
时检测到新版本自动执行。失败会在横幅提示原因，并提供「前往下载新版」
