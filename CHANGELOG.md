...
> 修一个真实事故:hub 一次给串行客户端派了 6 单,排在后面的单被 hub 判超时并重派,而老单还躺在本机
> 队列里没跑 —— 同一个 head 会被 review 两遍。

### 同一 PR 的待跑 job 只保留最新的一个

- 症状(2026-08-07 生产实测,`Liontech-tw/lion-trip-app-global`):hub 在 ~90 秒内派了 6 单;客户端
  一次只跑一单(实测 6–14min/单),排在第 5 位的 #696 等了 33 分钟都没轮到。**客户端排队期间不发
  `review_progress`**(它只在 clone/worktree/claude 三个阶段发),hub 眼里这单从派单起就彻底静默 →
  判失败、发「review 出错」、并重派一次。于是本机队列里同时躺着两个 #696(同一个 head `8b27c19a`),
  两单都会跑、都会往 GitHub 提一份 review。
- 现在 `review_job` 入队走 `enqueueJob()`:同一 `(repo, pr_num)` 的**尚未开跑**的 job 只保留最新的
  那个。被顶掉的那单**立刻回一条 `review_result`**(`exit_code: 0` + 说明文案,不带 verdict),让 hub
  马上收口 —— 而不是空等到超时,空等正是重派的成因。
- **已开跑的那单不动**:它可能正在评审老 head,合法的「再来一轮」理应排在它后面。去重键带 repo,
  不同项目的同号 PR 不会互相顶掉。
- 这是**兜底,不是根治**:根因在 hub 的派单策略(对同一 client 最多派 1 单在途,或超时死线从首个
  `review_progress` 起算)。客户端这层只保证「已经派进来的重复单不会被跑两遍」。
- macapp 同步改(`ReviewCoordinator.enqueue`),新增 3 个单测(`ReviewQueueTests`)。
