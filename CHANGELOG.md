...
> 两件事:**review 少烧一点额度**,以及**「空闲时自动更新」默认开启**。

- **review 子进程禁用 MCP**(`--strict-mcp-config`):review 用不到任何 MCP 工具
  (`/pr-review` 走 gh CLI;CodeGraph 有 `codegraph explore` shell 兜底),但 MCP 的工具定义
  每一轮都要进上下文。本机实测每轮省约 2100 token,按单次 review 15 轮算省约 3.5% 额度。
  Node 与 macapp 两边对等。
- **「空闲时自动更新」默认开启**(此前默认关)。老配置里存着的 `false` 无法区分「主动关掉」
  和「从没碰过」——此前默认就是关,且开关埋在设置页第二个 tab——故首次跑新版会**一次性
  强制打开**并写入 `autoUpdateDefaulted` 标记;之后自己在设置里关掉,不会再被翻回来。
  目的是让发版真正推得动:不必再挨个提醒成员点「更新并重启」。
